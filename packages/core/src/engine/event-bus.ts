...
export class EventBus {
  ...
  emit(event: TaskEvent, ctx: ContextBag | null = null): void {
    try {
      let target: EventBus | null = this;
      while (target !== null && target.handlers.size === 0) target = target.parent;
      ...
      // Telemetry context and observers are caller-owned and cannot affect task lifecycle.
    }
  }
