/**
 * Release evidence for the 0.6.0 external-consumer canary.
 *
...
await suite.proof(
  "REL-011",
  "external-consumer canary gates the 0.6.0 publication",
  "the packed 0.6.0 candidate policy crosses real provider and durable-receipt boundaries without publishing consumer details",
  async () => verifyReceipt("0.6.0"),
);
...
async function verifyReceipt(release) {
  const receipt = JSON.parse(await readFile(
    new URL(`../../../evidence/external-consumer-canary.v${release}.json`, import.meta.url),
    "utf8",
  ));
